...
f(1)=f0;
for n=1:N-1
    Rho(n+1)=Rho(n)+h*dRhodz(Rho(n),s(n),U);
     s(n+1)= s(n)+h*dsdz(Rho(n),f(n));
    f(n+1)=f(n)+h*dfdz(Rho(n),U, gamma);
end
...
xlabel('\xi')
ylabel('\rho')
title(sprintf('Plot for bacterial density at zdiff = %0.5f and zmax = %d',h,zh))
hold off
figure(2)
