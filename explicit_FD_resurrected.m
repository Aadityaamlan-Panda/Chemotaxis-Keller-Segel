...
for n=1:M
    for i=1:N
       ...
       d2c(n,i) = d2c(n,i) -(exp(-l(k)^2*t(n))*A(k)*sin(l(k)*x(i))*l(k)^2);
       end
       vc(n,i) = dc(n,i)/c(n,i);
    end
end
...
for n = 1:M-1
    for i=2:N-1
        ...
        %b(n+1,i)=b(n,i)+dt*(-mob_ratio*(b(n,i)*(vc(n,i+1)-vc(n,i))/dx+vc(n,i)*(b(n,i+1)-b(n,i))/dx));
    end
    b(:,N)=b(:,N-1);
end
...
for i=t_points./T.*(M-1)
    k=k+1;
    plot(x,mob_ratio.*vc(i+1,:),'LineWidth',2)
...
for i=l_points./L.*(N-1)
    k=k+1;
    plot(t,mob_ratio.*vc(:,i+1),'LineWidth',2)
